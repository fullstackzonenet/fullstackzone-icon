...
function resolveReplacementLegacyStyle({ nodePath, babel, state, macroName }) {
  ...
  if (parentPath.node.arguments) {
    ...
    if (
      ...
      nodePath.parentPath.node.arguments[0].value.startsWith('fsz-')
    ) {
      throw parentPath.buildCodeFrameError(`Don't begin the icon name with fsz-, just use ${nodePath.parentPath.node.arguments[0].value.slice(3)}`, MacroError)
    }
